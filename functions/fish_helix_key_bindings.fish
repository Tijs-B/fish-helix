...
function fish_helix_key_bindings --description 'helix-like key bindings for fish'
    ...
    # fish 4.8 changed __fish_shared_key_bindings to print its body as a string;
    # wrap it so $argv (the -M $mode flag) is visible inside the eval'd code.
    ...
    for mode in default visual
        ...
        bind -s --preset -M $mode _ "fish_helix_command trim_selections"
    ...
    # Match inner mode: select inside pair
    # Paired characters
    bind -s --preset -M match_inner -m default '(' "__fish_helix_select_pair '(' ')' inner; commandline -f repaint-mode"
    bind -s --preset -M match_inner -m default ')' "__fish_helix_select_pair '(' ')' inner; commandline -f repaint-mode"
    bind -s --preset -M match_inner -m default '[' "__fish_helix_select_pair '[' ']' inner; commandline -f repaint-mode"
    bind -s --preset -M match_inner -m default ']' "__fish_helix_select_pair '[' ']' inner; commandline -f repaint-mode"
    bind -s --preset -M match_inner -m default '{' "__fish_helix_select_pair '{' '}' inner; commandline -f repaint-mode"
    ...
    bind -s --preset -M match_inner -m default '`' "__fish_helix_select_pair '\`' '\`' inner; commandline -f repaint-mode"
    # Cancel
    bind -s --preset -M match_inner -m default \e cancel repaint-mode

    # Match around mode: select around pair (including delimiters)
    # Paired characters
